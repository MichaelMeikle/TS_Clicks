...
const char* ts3plugin_name() { return plugin->pluginName; }

/* Plugin version */
const char* ts3plugin_version() { return plugin->pluginVersion; }

/* Plugin API version. Must be the same as the clients API major version, else the plugin fails to load. */
int ts3plugin_apiVersion() { return plugin->apiVersion; }

/* Plugin author */
const char* ts3plugin_author() { return plugin->pluginAuthor; }

/* Plugin description */
const char* ts3plugin_description() { return plugin->pluginDescription; }
 ...
 */
int ts3plugin_init() {
	return plugin->init();  /* 0 = success, 1 = failure, -2 = failure but client will not show a "failed to load" warning */
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) {
	...
	plugin->proccessCommand(serverConnectionHandlerID, command);
...
const char* ts3plugin_infoTitle() { return plugin->pluginName; }
...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data) {
	...
	switch (type) {
		case PLUGIN_CLIENT:
			infoData = plugin->infoData(serverConnectionHandlerID, id, type, data);
			break;
		default:
			data = NULL;  /* Ignore */
			return;
	...
	*data = (char*)malloc(INFODATA_BUFSIZE * sizeof(char));
	snprintf(*data, INFODATA_BUFSIZE, infoData.c_str());
	infoData.clear();
}
 ...
 */
//Redefined enum for menu items in "plugin.h"
/*
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_GLOBAL,  GLOBAL_TOGGLE_PATROL,  "Toggle Patrol",  "icons/patrol.png");
	...
	*menuIcon = (char*)malloc(PLUGIN_MENU_BUFSZ * sizeof(char));
	_strcpy(*menuIcon, PLUGIN_MENU_BUFSZ, "icons/main_logo.png");
	//Hide most options on start
	//ts3Functions.setPluginMenuEnabled(pluginID, GLOBAL_TOGGLE_PATROL, 0);
	/*
...
void ts3plugin_initHotkeys(struct PluginHotkey*** hotkeys) {
	...
	//CREATE_HOTKEY("emergency", "Emergency Button");
	//CREATE_HOTKEY("radioStun", "Dispatch Radio Stun");
	//CREATE_HOTKEY("staffOverride", "Staff Override");
	END_CREATE_HOTKEYS;
...
void ts3plugin_onClientMoveEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* moveMessage) {
	if (newChannelID == NULL)
		plugin->onLeaveServerEvent(serverConnectionHandlerID, clientID, oldChannelID, newChannelID);
	else if(oldChannelID == NULL)
		plugin->onJoinServerEvent(serverConnectionHandlerID, clientID, oldChannelID, newChannelID);
	else
		plugin->onClientMoveEvent(serverConnectionHandlerID, clientID, oldChannelID, newChannelID);
}
...
void ts3plugin_onClientMoveTimeoutEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* timeoutMessage) {
	//Note sure if this event is called when there is a timeout failure or when the client actually TIMES OUT
	if (newChannelID == NULL)
		plugin->onLeaveServerEvent(serverConnectionHandlerID, clientID, oldChannelID, newChannelID);
	else if (oldChannelID == NULL)
		plugin->onJoinServerEvent(serverConnectionHandlerID, clientID, oldChannelID, newChannelID);
...
//Called right before recieved voice packet is played
void ts3plugin_onEditPlaybackVoiceDataEvent(uint64 serverConnectionHandlerID, anyID clientID, short* samples, int sampleCount, int channels) {
	...
	//This function recieves transmitted message, distorts the audio as appropriate, and if the client ID matches apporved speaker ID, data is played
	//If client ID does not match approved speaker ID (Someone listening to the proper channel), then data is muted. 
}

void ts3plugin_onEditPostProcessVoiceDataEvent(uint64 serverConnectionHandlerID, anyID clientID, short* samples, int sampleCount, int channels, const unsigned int* channelSpeakerArray, unsigned int* channelFillMask) {
	//*samples = 0;
	plugin->onEditPlaybackVoiceDataEvent(serverConnectionHandlerID, clientID, samples, sampleCount, channels, channelSpeakerArray, channelFillMask);
	//*channelFillMask = 0;
}
...
//Edits speaker voice data
void ts3plugin_onEditCapturedVoiceDataEvent(uint64 serverConnectionHandlerID, short* samples, int sampleCount, int channels, int* edited) {
	//What if this mutes client, data is streamed via a command callback for other clients, and packaged with speaker data
	//That way clients without plugin enabled will not hear anything, and the data can be packaged appropriately
	//*samples = 0;
	plugin->onCaputredVoiceDataEvent(serverConnectionHandlerID, samples, sampleCount, channels, edited);
	//sampleCount = 0;
	//channels = 0;
}
...
void ts3plugin_onClientSelfVariableUpdateEvent(uint64 serverConnectionHandlerID, int flag, const char* oldValue, const char* newValue) {
	plugin->onClientSelfVariableUpdate(serverConnectionHandlerID, flag, oldValue, newValue);
}
...
void ts3plugin_onPluginCommandEvent(uint64 serverConnectionHandlerID, const char* pluginName, const char* pluginCommand) {
	//printf("Plugin Command Received\n");
	//printf("Plugin Name: %s\n", pluginName);
	plugin->proccessCommand(serverConnectionHandlerID, pluginCommand);
}
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	//Causes a crash if activated without being connected to server
...
*/
struct TS3Functions* getFunctionHandler() {
	return &ts3Functions;
}
